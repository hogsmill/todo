...
const port = prod ? process.env.VUE_APP_PORT : 3200
...
MongoClient.connect(url, { useUnifiedTopology: true, maxIdleTimeMS: maxIdleTime }, (err, client) => {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('sendGetTasks', () => { dbStore.getTasks(db, io, debugOn) })

    socket.on('sendAddTask', (data) => { dbStore.addTask(db, io, data, debugOn) })
